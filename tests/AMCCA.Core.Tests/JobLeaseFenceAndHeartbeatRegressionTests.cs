...
public class JobLeaseFenceAndHeartbeatRegressionTests : IDisposable
{
    ...
    public async Task DEF016_DEF017_ZombieWorkerWithExpiredLease_CannotHeartbeat_Fail_Or_CompleteJob()
    {
        ...
        await actHeartbeatOrThrow.Should().ThrowAsync<AmccaException>()
            .Where(e => e.ErrorCode == AmccaErrors.Job002, "Heartbeat on expired lease must throw AMCCA-JOB-002");

        // 6. DEF-016: Stale Worker A attempts FailJobAsync with fence_token=1 -> must throw AMCCA-JOB-003
        ...
        await actFailA.Should().ThrowAsync<AmccaException>()
            .Where(e => e.ErrorCode == AmccaErrors.Job003, "Zombie worker cannot fail a job held by another worker (DEF-016)");

        // 7. Stale Worker A attempts CompleteJobOrThrowAsync with fence_token=1 -> must throw AMCCA-JOB-003
        ...
        await actCompleteA.Should().ThrowAsync<AmccaException>()
            .Where(e => e.ErrorCode == AmccaErrors.Job003, "Zombie worker cannot complete a job held by another worker");
